...
import { useBotPlayers } from "@/components/bot-players"
...
export function useMultiplayer({ room, userId, name, offline }: Options) {
  ...
  const bots = useBotPlayers({
    room,
    onBotMessage: (botName: string, message: string) => {
    ...
    onBotAction: (botId: string, action: { type: string; value: boolean }) => {
      // Handle bot actions if needed
    },
    gridCols: 20,
    gridRows: 16,
    walkableGrid: new Array(20 * 16).fill(true), // Simplified for now
  })

  const allPeers = useMemo(() => {
    const botPeers: Peer[] = bots.map((bot) => ({
      id: bot.id,
      name: bot.name,
      color: bot.color,
      x: bot.x,
    ...
    channel
      ...
      .on("broadcast", { event: "action" }, (payload) => {
        const data = payload.payload as {
          id: string
          type: "dance" | "sit" | "party" | "wave" | "laugh"
          value: boolean
        }
        if (!data) return
        if (data.type === "party") {
        ...
        if (data.id === userId) return
        if (data.type === "dance")
          setOthers((prev) => prev.map((o) => (o.id === data.id ? { ...o, dance: data.value } : o)))
        if (data.type === "sit")
          setOthers((prev) => prev.map((o) => (o.id === data.id ? { ...o, sit: data.value } : o)))
      })
      ...
      if (trimmed.length > MAX_LEN) {
        setMessages((prev) => [
          ...prev,
          {
            id: "sys-" + crypto.randomUUID(),
            author: "System",
            text: `Message is too long. Max ${MAX_LEN} characters.`,
            timestamp: Date.now(),
          },
        ])
        return
  ...
  const setDance = useCallback(
    async (value: boolean) => {
      ...
      await channelRef.current?.send({
        type: "broadcast",
        event: "action",
        payload: { id: userId, type: "dance", value },
      })
    },
  ...
  const setSit = useCallback(
    async (value: boolean) => {
      ...
      await channelRef.current?.send({
        type: "broadcast",
        event: "action",
        payload: { id: userId, type: "sit", value },
      })
    },
  ...
  const setPartySync = useCallback(
    async (value: boolean) => {
      ...
      await channelRef.current?.send({
        type: "broadcast",
        event: "action",
        payload: { id: userId, type: "party", value },
      })
    },
  ...
  const triggerWave = useCallback(async () => {
    ...
    await channelRef.current?.send({
      type: "broadcast",
      event: "action",
      payload: { id: userId, type: "wave", value: true },
    })
    setTimeout(async () => {
      await channelRef.current?.track({
        name,
      ...
      await channelRef.current?.send({
        type: "broadcast",
        event: "action",
        payload: { id: userId, type: "wave", value: false },
      })
    }, 2000)
  ...
  const triggerLaugh = useCallback(async () => {
    ...
    await channelRef.current?.send({
      type: "broadcast",
      event: "action",
      payload: { id: userId, type: "laugh", value: true },
    })
    setTimeout(async () => {
      await channelRef.current?.track({
        name,
      ...
      await channelRef.current?.send({
        type: "broadcast",
        event: "action",
        payload: { id: userId, type: "laugh", value: false },
      })
    }, 1800)
