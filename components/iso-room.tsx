...
import { drawCustomAvatar } from "@/lib/enhanced-avatar-renderer"
import type { AvatarCustomization } from "@/lib/avatar-customization"
import type { Facing } from "@/hooks/use-multiplayer"
...
interface IsoRoomProps {
  ...
  recentMessages: { author: string; text: string; timestamp: number }[]
  peers?: Peer[]
  onStep?: (step: { x: number; y: number; facing: "N" | "S" | "E" | "W" }) => void
  dancing?: boolean
  party?: boolean
  waving?: boolean
  laughing?: boolean
...
export default function IsoRoom({
  ...
  const [avatar, setAvatar] = useState<{ x: number; y: number; facing: "N" | "S" | "E" | "W" }>({
    x: 6,
    y: 6,
    facing: "S",
  })
  const pathRef = useRef<{ nodes: { x: number; y: number }[]; progress: number } | null>(null)
  const bubbleRef = useRef<{ text: string; expiresAt: number } | null>(null)
  const animRef = useRef(0) // gameplay anim speed
  const bgTimeRef = useRef(0) // background anim speed (independent of walking)
  const lastSentRef = useRef(0)
  const heldDirRef = useRef<{ dx: number; dy: number; facing: Peer["facing"] } | null>(null)
  ...
  const authorBubblesRef = useRef<Map<string, { text: string; expiresAt: number }>>(new Map())
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      const map: Record<string, { dx: number; dy: number; facing: Peer["facing"] }> = {
    ...
    function loop(now: number) {
      ...
      for (const { p, s } of peersSorted) {
        ...
        if (avatarCustomization) {
          const peerCustomization = { ...avatarCustomization, shirtColor: p.color }
          drawCustomAvatar(
            ctx,
            pX,
            pY,
...
function drawTile(
  ...
  ctx.lineTo(Math.round(px - hw) + 0.5, Math.round(py) + Math.round(hh) + 0.5)
  ctx.closePath()
  ctx.fillStyle = fill
  ctx.fill()
  ctx.lineWidth = 1
